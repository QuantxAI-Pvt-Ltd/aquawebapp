...
function debug(text) {
    ...
    // Use word boundaries so "IPSUM" doesn't trigger "SUM"
    const keywordRegex = /\b(TOTAL|AMOUNT|SUM|PAID|DUE|NET|BALANCE)\b/;
            ...
            if (matches) {
                for (const match of matches) {
                    ...
                    if (!isNaN(num) && num > maxAmount) {
                        // ignore the word "200" or similar from the cash line, since total <= cash.
                        // well, maxAmount will capture 200 instead of 107.6 if CASH has the keyword... wait, CASH is not in the regex.
                        maxAmount = num;
                    }
                }
            }
