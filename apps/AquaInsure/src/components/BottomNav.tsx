...
const BottomNav = () => {
  ...
  return (
    <div className="sticky bottom-0 z-50 w-full mt-auto bg-white/95 dark:bg-stone-900/95 backdrop-blur-md border-t border-stone-200/90 dark:border-stone-800 pb-[env(safe-area-inset-bottom,0px)] shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
      <nav className="h-[64px] px-2 flex items-center justify-between">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className="relative flex flex-col items-center justify-center flex-1 h-full py-2 transition-all duration-150 active:scale-95 touch-manipulation group"
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute top-0 w-9 h-[2.5px] rounded-full bg-teal-600"
